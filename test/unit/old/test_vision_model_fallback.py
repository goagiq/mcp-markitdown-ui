#!/usr/bin/env python3
"""
Test script for vision model fallback with available minicpm-v model.
"""
...
def test_vision_model_fallback():
    """Test the enhanced PDF OCR converter with vision model fallback."""
    
    # PDF file to test
    pdf_path = "input/Publication of Unclassified Intelligence Analysis Products IPM.pdf"
    
    try:
        # Initialize the enhanced converter with minicpm-v as primary model
        logger.info("Initializing Enhanced PDF OCR Converter with minicpm-v...")
        converter = EnhancedPdfOcrConverter(
            vision_model="minicpm-v:latest",  # Use the available model
        ...
        with open(pdf_path, 'rb') as file_stream:
            ...
            if converter.accepts(file_stream, stream_info):
                ...
                if result and result.markdown:
                    ...
                    # Save the result
                    output_path = "output/Publication of Unclassified Intelligence Analysis Products IPM_vision_fallback.markdown"
